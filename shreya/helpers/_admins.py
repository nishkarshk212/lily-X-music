...
def can_manage_vc(func):
    ...
    async def wrapper(_, update: types.Message | types.CallbackQuery, *args, **kwargs):
        ...
        # Check for skip permission if the action is a skip
        is_skip = False
        if isinstance(update, types.Message):
            if update.text:
                cmd = update.text.split()[0].lower().replace("/", "")
                if cmd.split("@")[0] in ["skip", "next"]:
                    is_skip = True
        elif isinstance(update, types.CallbackQuery):
            if update.data and "skip" in update.data:
                is_skip = True

        if is_skip:
            skip_mode = await db.get_skip_mode(chat_id)
            if not skip_mode: # If skip_mode is False (Everyone), allow everyone
                return await func(_, update, *args, **kwargs)
