...
from shreya import app, config, db, logger, queue, yt
from shreya.helpers import utils, Track


async def background_download(track: Track) -> None:
    if not track.file_path:
        try:
            track.file_path = await yt.download(track.id, video=track.video)
        except Exception as e:
            logger.error(f"Background download failed for {track.title}: {e}")
