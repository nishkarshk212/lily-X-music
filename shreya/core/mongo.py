...
from shreya import config, logger, userbot
...
class MongoDB:
    ...
    async def get_admins(self, chat_id: int, reload: bool = False) -> list[int]:
        from shreya.helpers._admins import reload_admins
    ...
    async def set_skip_mode(self, chat_id: int, remove: bool = False) -> None:
        if remove and chat_id in self.skip_mode:
            self.skip_mode.remove(chat_id)
        elif not remove:
            self.skip_mode.append(chat_id)
        await self.chatsdb.update_one(
            {"_id": chat_id},
            {"$set": {"skip_mode": not remove}},
            upsert=True,
        )
